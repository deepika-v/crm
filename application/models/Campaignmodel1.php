...
{  
   $sql = "SELECT DISTINCT (crm_campaign.CampaignID), crm_campaign.CampaignName 
           ...
           WHERE crm_campaigncontacts.AssignedTo = '$assignid' ORDER BY crm_campaign.CampaignName";
...
function get_productlist(){
  $sql = "SELECT  a.ProductId, a.ProductName, a.CategoryIds, GROUP_CONCAT(b.CategoryName ORDER BY b.CategoryId DESC) as categoryname
       FROM    Product as a
       INNER JOIN ProductCategory as b
       ON FIND_IN_SET(b.CategoryId, a.CategoryIds)
...
function get_categorylist(){
  $sql = "SELECT DISTINCT CategoryId, CategoryName FROM `ProductCategory`";
       $q = $this->db->query($sql);
          if($q->num_rows() > 0)
          {
            foreach ($q->result() as $row)
